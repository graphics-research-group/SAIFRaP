...
class AutoregressiveTransformer(BaseAutoregressiveTransformer):
    ...
    @torch.no_grad()
    def add_object(self, room_mask, class_label, boxes=None, device="cpu", translation=None):
        boxes = dict(boxes.items())

        # Handle class_label formatting
        if isinstance(class_label, int) or isinstance(class_label, np.integer):
            one_hot = torch.eye(self.n_classes, device=device)
        ...
        # Override translation if specified
        if translation is not None:
            if not torch.is_tensor(translation):
                translation = torch.tensor(translation, dtype=torch.float32)
            if translation.dim() == 1:
                translation = translation.unsqueeze(0).unsqueeze(0)  # shape (1, 1, 3)
            box["translations"] = translation.to(device)

        # Add decoded box to sequence
        for k in box.keys():
            boxes[k] = torch.cat([boxes[k], box[k]], dim=1)
